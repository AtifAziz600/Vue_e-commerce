<template>
  ...
  >
    ...
    >
      ...
      <RouterLink to="/" class="flex items-center gap-2 cursor-pointer">
        ...
        <span class="text-xl font-bold text-gray-900 hidden sm:inline">What a Market</span>
      </RouterLink>

      <!-- Search Bar -->
      <form class="flex flex-grow border border-gray-200 rounded overflow-hidden bg-white shadow-sm">
        <select class="bg-[#4094F7] text-white font-bold text-sm px-3 py-2 md:px-1 focus:outline-none md:block md:w-auto hidden">
        ...
        <button type="submit" class="bg-yellow-500 px-2 md:px-2  text-sm text-white hover:bg-yellow-600 transition duration-200">
      ...
      <div class="md:flex items-center gap-4 hidden">
        <div class="flex justify-center items-center gap-4">
          <RouterLink to="/order-history" class="flex items-center gap-1 cursor-pointer hover:bg-gray-100 px-3 py-2 rounded transition duration-200">
          ...
          <RouterLink to="/wishlist" class="flex items-center gap-1 cursor-pointer hover:bg-gray-100 px-3 py-2 rounded transition duration-200">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-600" viewBox="0 0 20 20" fill="currentColor">
              <path fill-rule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clip-rule="evenodd" />
            </svg>
            <span>Favorites</span>
          </RouterLink>
          <RouterLink to="/cart-order" class="relative flex items-center gap-1 cursor-pointer hover:bg-gray-100 px-3 py-2 rounded transition duration-200">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-600" viewBox="0 0 20 20" fill="currentColor">
            ...
            <RouterLink to="/cart-order" class="absolute -top-1 -right-1 bg-red-500 text-white text-xs w-5 h-5 flex items-center justify-center rounded-full">{{ cartCount }}</RouterLink>
            <span>Cart</span>
          </RouterLink>
          <RouterLink to="/login" class="cursor-pointer border px-3 py-2 rounded hover:bg-gray-100 transition duration-200">
            <span>Sign in</span>
          </RouterLink>
        </div>
      </div>
      ...
      <transition name="fade" class="overflow-auto">
        <div v-if="isSidebarOpen" class="fixed inset-0 z-50 bg-black bg-opacity-40 flex">
          <div class="bg-white w-64 h-full shadow-lg p-6 flex flex-col">
            <div class="flex justify-between items-center mb-6">
              <span class="text-lg font-bold">Menu</span>
              <button @click="isSidebarOpen = false" class="p-2 rounded hover:bg-gray-200">
            ...
            <nav class="flex flex-col gap-4">
              <a v-for="item in NavItems" :key="item.name" :href="item.link" class="py-2 px-3 rounded hover:bg-gray-100 transition duration-200">
                {{ item.name }}
              </a>
            </nav>
            <div class="mt-8 flex flex-col gap-3">
              <RouterLink to="/cart-order" class="flex items-center gap-2 cursor-pointer hover:bg-gray-100 px-3 py-2 rounded transition duration-200">
                <Icon icon="mdi:cart" class="h-5 w-5 text-gray-500" />
                <span>Cart</span>
              </RouterLink>
              <RouterLink to="/login" class="flex items-center gap-2 cursor-pointer hover:bg-gray-100 px-3 py-2 rounded transition duration-200">
  ...
  <nav class="container mx-auto flex items-center justify-between">
    ...
    <div class="flex items-center gap-2 text-gray-700 hover:text-black cursor-pointer transition">
      <Icon icon="mdi:map-marker" class="h-5 w-5 text-red-500" />
      <div class="leading-tight">
        <p class="text-xs text-gray-500">Delivering to</p>
        <p class="text-sm font-semibold">Washington 56972</p>
      </div>
    </div>
