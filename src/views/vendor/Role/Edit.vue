...
const getRole = async () => {
  const res = await sendRequest({
    url:`/v1/get-role-with-permissions/${route.params.id}`
  })
  createRole.value.roleName = res.data.name;
  res.data.permissions.map(item => createRole.value.selectedPermissions.push(item.name))
}
...
const saveNewRole = async () => {
  const res = await sendRequest({
    url:`/v1/update-role/${route.params.id}`,
    method:'POST',
                ...
                <tr class="border-b last:border-b-0" v-for="(module, index, i) in permissions" :key="module.id">
                  ...
                  <td class="py-5">
                    <div :class="`manage-permission-${i}`">
                      <div class="flex items-center mb-2" v-for="(permission, index) in module" :key="permission.name">
                        <input
                            class="form-check-input h-4 w-4 border-gray-300 disabled:bg-primary-300 rounded focus:outline-none focus:border-none focus:shadow-none focus:ring-0"
                            type="checkbox"
                            :disabled="loadCreateRole"
