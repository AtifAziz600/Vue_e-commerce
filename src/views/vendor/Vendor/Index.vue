            ...
            <tr :v-if="!loading && items.data.length" class="bg-white border-b" v-for="(item, index) in items?.data" :key="item.id"
              ...
              <th scope="row" class="px-6 py-1 w-[40%] font-medium text-gray-900 whitespace-nowrap dark:text-white">
                <div class="flex gap-4">
                  <img class="w-7 h-7 rounded-full object-contain" :src="item.user.avatar"
                       @error="event => event.target.src=`https://ui-avatars.com/api/?name=${item.user.name}`" alt="">
                  <div>
                    <p class="text-xs font-bold capitalize text-primary-800">{{ item?.user.name }}</p>
                    <p class="text-[10px] font-normal leading-none text-primary-400">{{ item?.user.email }}</p>
                  </div>
                </div>
              </th>
              <td class="px-6 py-1">
